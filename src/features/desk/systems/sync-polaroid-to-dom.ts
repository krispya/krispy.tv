...
import { Dragging, Polaroid, Position, Ref, Rotation, StackIndex } from '../traits/index.js';
...
export function syncPolaroidToDOM(world: World) {
  world
    ...
    .updateEach(([_polaroid, position, rotation, ref, stackIndex], entity) => {
      const cssZIndex = getPolaroidZIndex(stackIndex, entity.has(Dragging));
...
function getPolaroidZIndex(stackIndex: { value: number }, isDragging: boolean) {
